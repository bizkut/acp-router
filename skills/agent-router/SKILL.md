...
The alpha MCP server exposes safe local registry tools. It discovers installed agents, records jobs and sessions, writes JSONL job logs, tails job events for polling progress, captures current git worktree state, and can run OpenCode through native ACP plus Claude Code, Cursor Agent, and Codex CLI through CLI fallback. Runnable dispatch tools default to `launchExternalAgents=true` for existing absolute worktrees; pass `launchExternalAgents=false` on a single call when the user wants record-only behavior.
